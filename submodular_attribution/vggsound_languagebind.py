...
def parse_args():
    ...
    parser.add_argument('--eval-list',
                        type=str,
                        default='datasets/vggsound/val_languagebind_309_false.txt',
                        help='Datasets.')
    parser.add_argument('--partition-method',
                        type=str,
                        default='patch',
                        choices=['patch'],
                        help='Audio spectrogram partition method.')
    ...
    parser.add_argument('--save-dir',
                        type=str, default='./submodular_results/vggsound-languagebind-efficientv2-debug/',
...
def main(args):
    ...
    smdl = AudioSubModularExplanationEfficientPlus(
        ...
        class_names=vggsound_classes)

    with open(args.eval_list, "r") as f:
        infos = f.read().split('\n')

    mkdir(args.save_dir)
    save_dir = os.path.join(args.save_dir, "{}-{}-{}-{}-{}-pending-samples-{}-division-{}".format(args.partition_method, args.lambda1, args.lambda2, args.lambda3, args.lambda4, args.pending_samples, args.grad_partition_size))
    ...
    for info in tqdm(select_infos):
        ...
        saved_json_file["partition_method"] = args.partition_method
        saved_json_file["partition_size"] = [args.grad_partition_size, args.grad_partition_size]
